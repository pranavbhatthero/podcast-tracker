...
- "price_ticker"  : Yahoo Finance ticker if applicable (e.g. "MU", "NVDA", "BTC-USD"), else null
...
for i, episode in enumerate(bg2_eps):
    ...
    segs  = parse_vtt(vtt)
    calls = extract(client, episode, segs, BG2_SPEAKERS)
    print(f"         → {len(calls)} calls found")
    new_preds.extend(calls)

# ── Merge ─────────────────────────────────────────────────────────────────────
seen_keys = {(p.get('episode_url',''), p.get('timestamp','')) for p in existing}
